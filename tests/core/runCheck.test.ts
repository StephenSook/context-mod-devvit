...
describe('runCheck', () => {
  it('AND with 2 hits → triggered, returns actions', async () => {
    const r = await runCheck({
      ...
      rules: [ruleHit, ruleHit], actions: [removeAction],
    }, baseItem, baseAuthor);
  ...
  it('OR with 1 hit + 1 miss → triggered', async () => {
    const r = await runCheck({
      name: 'c', combinator: 'OR', rules: [ruleHit, ruleMiss],
    }, baseItem, baseAuthor);
    expect(r.triggered).toBe(true);
  });
  ...
  it('checkName is echoed back', async () => {
    const r = await runCheck({
      name: 'block-scam', combinator: 'AND', rules: [ruleHit],
    }, baseItem, baseAuthor);
