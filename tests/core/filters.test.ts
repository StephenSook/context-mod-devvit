...
describe('Codex H5 — invalid filter regex MUST not throw (fail-CLOSED to false)', () => {
  ...
  it('valid regex still functions normally', () => {
    ...
    expect(
      passesFilters({ itemIs: { titleMatches: '^bye$' } }, baseItem, baseAuthor)
    ).toBe(false);
  });
});
...
describe('passesFilters — authorIs predicates', () => {
  it('nameIn positive', () => {
    expect(
      passesFilters({ authorIs: { nameIn: ['u'] } }, baseItem, baseAuthor)
    ).toBe(true);
  });
  it('nameIn negative', () => {
    expect(
      passesFilters({ authorIs: { nameIn: ['other'] } }, baseItem, baseAuthor)
    ).toBe(false);
  });
  it('nameNotIn negative (author is in the not-in list → reject)', () => {
    expect(
      passesFilters({ authorIs: { nameNotIn: ['u'] } }, baseItem, baseAuthor)
    ).toBe(false);
  });
  it('ageMinSec — below min rejects', () => {
    expect(
      passesFilters(
        { authorIs: { ageMinSec: 1_000_000 } },
        baseItem,
        baseAuthor
      )
    ).toBe(false);
  ...
  it('ageMinSec — at/above min passes', () => {
    expect(
      passesFilters({ authorIs: { ageMinSec: 86_400 } }, baseItem, baseAuthor)
    ).toBe(true);
  });
  it('linkKarmaMax — over max rejects', () => {
    expect(
      passesFilters({ authorIs: { linkKarmaMax: 50 } }, baseItem, baseAuthor)
    ).toBe(false);
  });
  it('commentKarmaMin — under min rejects', () => {
    expect(
      passesFilters(
        { authorIs: { commentKarmaMin: 500 } },
        baseItem,
        baseAuthor
      )
    ).toBe(false);
  ...
  it('isMod predicate', () => {
    expect(
      passesFilters({ authorIs: { isMod: true } }, baseItem, baseAuthor)
    ).toBe(false);
    expect(
      passesFilters({ authorIs: { isMod: false } }, baseItem, baseAuthor)
    ).toBe(true);
  });
  it('flairTextIn rejects null author flair', () => {
    expect(
      passesFilters(
        { authorIs: { flairTextIn: ['VIP'] } },
        baseItem,
        baseAuthor
      )
    ).toBe(false);
...
describe('passesFilters — itemIs predicates', () => {
  it('over18 positive', () => {
    expect(
      passesFilters({ itemIs: { over18: false } }, baseItem, baseAuthor)
    ).toBe(true);
  });
  it('over18 negative', () => {
    expect(
      passesFilters({ itemIs: { over18: true } }, baseItem, baseAuthor)
    ).toBe(false);
  });
  it('scoreMin — under min rejects', () => {
    expect(
      passesFilters({ itemIs: { scoreMin: 100 } }, baseItem, baseAuthor)
    ).toBe(false);
  });
  it('scoreMax — over max rejects', () => {
    expect(
      passesFilters({ itemIs: { scoreMax: 1 } }, baseItem, baseAuthor)
    ).toBe(false);
  });
...
describe('passesFilters — combined item + author', () => {
  it('both pass → true', () => {
    expect(
      passesFilters(
        { authorIs: { isMod: false }, itemIs: { over18: false } },
        baseItem,
        baseAuthor
      )
    ).toBe(true);
  ...
  it('author fails → false even when item passes', () => {
    expect(
      passesFilters(
        { authorIs: { isMod: true }, itemIs: { over18: false } },
        baseItem,
        baseAuthor
      )
    ).toBe(false);
