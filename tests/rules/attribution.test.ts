...
const histWithDomains = (domains: string[]) => ({
  ...
  posts: domains.map((d, i) => ({
    id: `t3_${i}`, subredditName: 's', url: `https://${d}/x`, domain: d, createdAtMs: 0,
  })),
  comments: [],
});
...
describe('runAttributionRule', () => {
  it('triggers when 100% of posts match the domain', async () => {
    getAuthorHistoryMock.mockResolvedValueOnce(histWithDomains([
      'example.com', 'example.com', 'example.com', 'example.com', 'example.com',
    ]));
    const rule: AttributionRule = { kind: 'attribution', domains: ['example.com'], domainPercent: 100 };
    expect((await runAttributionRule(rule, 'alice')).triggered).toBe(true);
  });

  it('does not trigger when below the percent', async () => {
    getAuthorHistoryMock.mockResolvedValueOnce(histWithDomains([
      'example.com', 'example.com', 'other.com', 'other.com', 'other.com',
    ]));
    const rule: AttributionRule = { kind: 'attribution', domains: ['example.com'], domainPercent: 50 };
  ...
  it('substring + case-insensitive — www.youtube.com counts for "youtube.com"', async () => {
    getAuthorHistoryMock.mockResolvedValueOnce(histWithDomains([
      'www.YouTube.com', 'm.youtube.com', 'youtube.com', 'reddit.com', 'reddit.com',
    ]));
    const rule: AttributionRule = { kind: 'attribution', domains: ['youtube.com'], domainPercent: 50 };
    expect((await runAttributionRule(rule, 'alice')).triggered).toBe(true);
  });

  it('multiple domains — union match', async () => {
    getAuthorHistoryMock.mockResolvedValueOnce(histWithDomains([
      'youtube.com', 'twitch.tv', 'kick.com', 'other.com', 'other.com',
    ]));
  ...
  it('honors a custom minPosts override', async () => {
    ...
    const rule: AttributionRule = {
      kind: 'attribution', domains: ['example.com'], domainPercent: 100, minPosts: 2,
    };
    expect((await runAttributionRule(rule, 'alice')).triggered).toBe(true);
  });

  it('no-ops on empty author name (no Reddit hit)', async () => {
    const rule: AttributionRule = { kind: 'attribution', domains: ['x.com'], domainPercent: 1 };
  ...
  it('no-ops on empty domains list', async () => {
    const rule: AttributionRule = { kind: 'attribution', domains: [], domainPercent: 1 };
    expect((await runAttributionRule(rule, 'alice')).triggered).toBe(false);
    expect(getAuthorHistoryMock).not.toHaveBeenCalled();
  });
