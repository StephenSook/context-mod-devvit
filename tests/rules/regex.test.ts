...
const baseItem: Item = {
  id: 't3_a', title: 'free crypto giveaway scam', body: 'body text',
  url: 'https://example.com/path', author: 'u', age: 0, score: 0,
...
describe('runRegexRule', () => {
  it('matches title by default', () => {
    expect(runRegexRule({ kind: 'regex', pattern: 'scam', flags: 'i' }, baseItem).triggered).toBe(true);
  });
  ...
  it('targets body when specified', () => {
    expect(runRegexRule({ kind: 'regex', pattern: 'body', target: 'body' }, baseItem).triggered).toBe(true);
    expect(runRegexRule({ kind: 'regex', pattern: 'body', target: 'title' }, baseItem).triggered).toBe(false);
  });

  it('targets url when specified', () => {
    expect(runRegexRule({ kind: 'regex', pattern: 'example\\.com', target: 'url' }, baseItem).triggered).toBe(true);
  });

  it('respects multiline flag', () => {
    const multi: Item = { ...baseItem, title: 'line1\nline2' };
    expect(runRegexRule({ kind: 'regex', pattern: '^line2', flags: 'm' }, multi).triggered).toBe(true);
