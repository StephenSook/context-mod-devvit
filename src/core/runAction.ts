...
function payloadDigest(a: Action): string {
  switch (a.kind) {
    case 'remove':    return `spam=${a.isSpam ?? false}`;
    case 'approve':   return '';
    case 'comment':   return `tpl=${a.template}`;
    case 'lock':      return '';
    case 'report':    return `reason=${a.reason}`;
    case 'ban':       return `dur=${a.duration ?? 0}|reason=${a.reason ?? ''}|note=${a.note ?? ''}`;
    case 'userFlair': return `text=${a.text ?? ''}|css=${a.cssClass ?? ''}`;
  }
}
...
export async function runAction(action: Action, ctx: ActionContext): Promise<ActionResult> {
  ...
  try {
    switch (action.kind) {
      ...
      case 'approve':   await runApprove(action, ctx); break;
      case 'comment':   await runComment(action, ctx); break;
      case 'lock':      await runLock(action, ctx); break;
      case 'report':    await runReport(action, ctx); break;
